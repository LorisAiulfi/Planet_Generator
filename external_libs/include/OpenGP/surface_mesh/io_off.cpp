                    ...
                    const bool has_colors)
{
    ...
    for (i=0; i<nV && !feof(in); ++i)
    {
        ...
        // position
        items = sscanf_s(lp, "%f %f %f%n", &p[0], &p[1], &p[2], &nc);
        assert(items==3);
        ...
        if (has_normals)
        {
            if (sscanf_s(lp, "%f %f %f%n", &n[0], &n[1], &n[2], &nc) == 3)
            {
                normals[v] = n;
            }
            lp += nc;
        ...
        if (has_colors)
        {
            if (sscanf_s(lp, "%f %f %f%n", &c[0], &c[1], &c[2], &nc) == 3)
            {
        ...
        if (has_texcoords)
        {
            items = sscanf_s(lp, "%f %f%n", &t[0], &t[1], &nc);
            assert(items == 2);
            texcoords[v][0] = t[0];
            texcoords[v][1] = t[1];
            lp += nc;
    ...
    for (i=0; i<nF; ++i)
    {
        ...
        // #vertices
        items = sscanf_s(lp, "%d%n", (int*)&nV, &nc);
        assert(items == 1);
        vertices.resize(nV);
        lp += nc;
        ...
        for (j=0; j<nV; ++j)
        {
            items = sscanf_s(lp, "%d%n", (int*)&idx, &nc);
            assert(items == 1);
...
bool write_off(const Surface_mesh& mesh, const std::string& filename)
{
	...
	FILE* out;
	errno_t err	= fopen_s(&out, filename.c_str(), "w");
    if (!out)
        return false;
